...
class Graph{
	...
	public void DFS(Dictionary<string, int> disc,  Dictionary<string, int> low, string parent, List<string> visitedNode, string startNode, int timer, Dictionary<string, string> result){
		...
		if(map.ContainsKey(startNode)){
			...
			foreach(var child in childs){
				...
				if(!visitedNode.Contains(child)){
					...
					if(low[child] > disc[startNode]){
						result.Add(startNode, child);
					}
				}
				else {
